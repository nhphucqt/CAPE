...
from EdgeCape.models.pam.utils import build_transformer
...
class PoseHead(nn.Module):
    ...
    def __init__(self,
                 ...
                 positional_encoding=dict(
                     type='PAM_SinePositionalEncoding',
                     num_feats=128,
                     normalize=True),
    ...
    def forward(self, x, feature_s, target_s, mask_s, skeleton):
        ...
        outs_dec, initial_proposals, out_points, similarity_map, query_embed, attn_maps = self.transformer(x,
                                                                                   masks,
                                                                                   support_keypoints,
                                                                                   pos_embed,
        ...
        return torch.stack(output_kpts, dim=0), initial_proposals, similarity_map, query_embed, attn_maps
...
class PoseHeadV4(nn.Module):
    ...
    def __init__(self,
        ...
        if extra is not None and not isinstance(extra, dict):
            raise TypeError('extra should be dict or None.')
        """Initialize layers of the transformer head."""
        self.input_proj = Conv2d(self.in_channels, self.embed_dims, kernel_size=1, bias=False)
        # self.input_proj = Conv2d(self.in_channels, self.embed_dims, kernel_size=1)
        # self.query_proj = Linear(self.in_channels, self.embed_dims)
        # Instantiate the proposal generator and subsequent keypoint branch.
        kpt_branch = TokenDecodeMLP(
    ...
    def heatmap_loss(
        self,
        similarity_map,   # [bs, nq, h, w]  (already softmaxed)
        target_heatmap,   # [bs, nq, sh, sw]
        target_weight,    # [bs, nq, 1]
        normalizer        # [bs]
    ):
        """
        Heatmap supervision on similarity probability maps.
        """

        bs, nq, h, w = similarity_map.shape

        # print("similarity_map:", similarity_map.min().item(), similarity_map.max().item(), similarity_map.mean().item())

        # resize target heatmap to match similarity map resolution
        target_heatmap = F.interpolate(
            target_heatmap,
            size=(h, w),
            mode='bilinear',
        ...
        # normalize target heatmap so that sum over spatial dims = 1
        target_heatmap = target_heatmap / (
            target_heatmap.sum(dim=(-1, -2), keepdim=True) + 1e-10
        )

        # print("target_heat:", target_heatmap.min().item(), target_heatmap.max().item(), target_heatmap.mean().item())

        # MSE between probability maps
        # loss = F.mse_loss(
        #     similarity_map,
        #     target_heatmap,
        #     reduction='none'   # [bs, nq, h, w]
        # )

        loss = F.kl_div(
            (similarity_map + 1e-10).log(),
            target_heatmap,
            reduction='none'
        )

        # apply keypoint visibility weights
        loss = loss * target_weight[:, :, :, None]  # [bs, nq, h, w]

        # average over spatial dimensions
        loss = loss.flatten(2).mean(dim=-1)  # [bs, nq]

        # normalize per-sample by number of visible joints
        loss = loss.sum(dim=-1) / normalizer  # [bs]

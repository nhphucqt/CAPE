...
from EdgeCape.models.pam.backbones.swin_utils import load_pretrained


@POSENETS.register_module()
class SemanticMatchingModel(BasePose):
    ...
    def forward_train(self,
        ...
        # output, initial_proposals, similarity_map, mask_s, feature_q, feature_s = self.predict(
        #     img_s, target_s, target_weight_s, img_q, img_metas)
    ...
    def forward_test(self,
        ...
        output, initial_proposals, similarity_map, _, adj, feature_q, feature_s = self.predict(img_s, target_s, target_weight_s, img_q, img_metas)
        ...
        result.update({
            "points":
                torch.cat((initial_proposals[None], output)).cpu().numpy()
        })
    ...
    def predict(self,
        ...
        # Only keep points that are visible throught all support images
        mask_s = target_weight_s[0]
        for target_weight in target_weight_s:
            mask_s = mask_s * target_weight

        output, initial_proposals, similarity_map, adj = self.keypoint_head(feature_q, feature_s, target_s, mask_s, skeleton)
        return output, initial_proposals, similarity_map, mask_s, adj, feature_q, feature_s

...
class EdgeCapeRefined(BasePose):
    """
    ...
    def __init__(self,
        ...
        self.keypoint_refine_head_module = RefineHead()
    ...
    def forward(self,
                ...
                target_coarse_s=None,
                target_q=None,
                target_weight_q=None,
                target_coarse_q=None,
                img_metas=None,
                return_loss=True,
                **kwargs):
        ...
        if return_loss:
            ...
            return self.forward_test(img_s, target_s, target_weight_s, target_coarse_s, img_q,
                                     target_q, target_weight_q, target_coarse_q, img_metas,
                                     **kwargs)
    ...
    def forward_train(self,
        ...
        output, initial_proposals, similarity_map, mask_s, reconstructed_keypoints, adj, feature_q, feature_s = self.predict(img_s,
                                                                                                       target_s,
                                                                                                       target_weight_s,
                                                                                                       img_q,
                                                                                                       img_metas,
    ...
    def forward_test(self,
        ...
        output, initial_proposals, similarity_map, mask_s, reconstructed_keypoints, adj, feature_q, feature_s = self.predict(img_s,
                                                                                                       ...
                                                                                                       img_q,
                                                                                                       img_metas,
                                                                                                       target_coarse_s=target_coarse_s,
                                                                                                       )
        ...
        result.update({
            "similarity_map": similarity_map.detach().cpu().numpy() if similarity_map is not None else None,
            "feature_q": feature_q.detach().cpu(),
            "feature_s": [f.detach().cpu() for f in feature_s],
        })
    ...
    def predict(self,
        ...
        # print(f"{len(target_weight_s) = }, {target_weight_s[0].shape = }")
        # print(f"{len(target_coarse_s) = }, {target_coarse_s[0].shape = }")

        # print(f"{len(target_s) = }, {target_s[0].shape = }")

        # print(f"{len(img_metas) = }")
        # print(f"{img_metas[0] = }")


        target_s_ori = []
        for info in img_metas:
            target_s_ori.append(torch.tensor(np.array(info['sample_joints_3d'])).to(img_q.device)[:, :, :2])
        target_s_ori = torch.stack(target_s_ori, dim=0) # [bs, n, K, 2]

        mask_s = target_weight_s[0]
        coarse_s = target_coarse_s[0]
        ...
        output = self.keypoint_refine_head_module(target_s_ori, output, coarse_s, mask_s)

        # print(f"{output.shape = }")

        return output, initial_proposals, similarity_map, mask_s, reconstructed_keypoints, adj, feature_q, feature_s

...
class ProposalGenerator(nn.Module):
    ...
    def forward(self, query_feat, support_feat, spatial_shape):
        ...
        local_similarity_softmax = similarity_softmax[:, :, :,
                                                      None] * local_max_mask  # (bs, nq, w*h, 1)
...
class CapeFormerTwoStageSupportRefineTransformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed, support_order_embed,
        ...
        return hs.transpose(1, 2), memory.permute(1, 2, 0).view(
            bs, c, h,
            w), initial_proposals_for_loss, out_points, similarity_map,
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.ref_point_head = MLP(
            ...
            2)  # this MLP will process the positional embedding for each query
    ...
    def forward(self,
        ...
        for lidx, layer in enumerate(self.layers):
            if lidx == 0:  # use positional embedding form inital proposals
                ...
                query_pos_embed = position_embedding.forward_coordinates(
                    query_coordinates)  # recalculate the positional embedding

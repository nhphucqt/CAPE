...
class CapeFormerTwoStageHead(nn.Module):
    '''
    ...
    def __init__(self,
        ...
        # self.init_weights() # it's ok as init_weights will be called during constructing the whole keypoint detector.

        if support_embedding_type == 'fixed':
            self.support_embedding = None
        elif support_embedding_type == 'learned':
            self.support_embedding = nn.Embedding(num_support, self.embed_dims)
            nn.init.normal_(self.support_embedding.weight.data)
        else:
    ...
    def forward(self, x, feature_s, target_s, mask_s):
        ...
        x = self.input_proj(x)  # [bs, dim, h64, w64], /64 resolution.
        bs, dim, h, w = x.shape

        if self.support_embedding:
            support_order_embedding = self.support_embedding.weight
            support_order_embedding = support_order_embedding.transpose(
                0, 1)[None, :, :].repeat(bs, 1, 1).unsqueeze(
                    -2)  # [bs, embed_dim, 1, num_query]
        else:
            masks = x.new_zeros((bs, 1, target_s[0].shape[1])).to(torch.bool)
            support_order_embedding = self.positional_encoding(
                masks)  # [bs, embed_dim, 1, num_query]

        if self.support_order_dropout != -1:
            support_order_embedding = support_order_embedding.transpose(1, 3)
            support_order_embedding = torch.nn.functional.dropout2d(
                support_order_embedding, self.support_order_dropout,
                self.training)
            support_order_embedding = support_order_embedding.transpose(1, 3)
        ...
        for feature, target in zip(
            ...
            query_embed = target.flatten(2) @ resized_feature.flatten(
                2).permute(0, 2, 1)  # [bs, num_query, backbone_dim]
        ...
        # memory: [bs, c, h, w]
        outs_dec, memory, initial_proposals, out_points, similarity_map = self.transformer(
            x, masks, query_embed, pos_embed, support_order_embedding,
        ...
        return torch.stack(
            output_kpts, dim=0
        ), initial_proposals, similarity_map  # [num_dec, bs, num_query, 2]
    ...
    def heatmap_loss(self, similarity_map, target_heatmap, target_weight,
        ...
        target_heatmap = target_heatmap / (
            ...
            1e-10)[:, :, None, None]  # make sure sum of each query is 1

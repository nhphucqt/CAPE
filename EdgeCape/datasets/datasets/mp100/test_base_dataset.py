...
class TestBaseDataset(Dataset, metaclass=ABCMeta):
    ...
    def __getitem__(self, idx):
        ...
        # print(Xall.keys())
        # print(f"{Xall['img_s'][0].shape = }")
        # print(f"{Xall['img_q'].shape = }")
        # # Convert the first support heatmap to keypoints by finding the peak position for each joint
        # target_s = Xall['target_s'][0]  # shape: [num_joints, map_h, map_w]
        # keypoints = []
        # for joint_heatmap in target_s:
        #     idx = np.unravel_index(np.argmax(joint_heatmap), joint_heatmap.shape)
        #     keypoints.append(idx[::-1])  # (x, y) order
        # print(f"Support keypoints (from heatmap peaks): {keypoints}")
        # print(f"{Xall['target_q'].shape = }")
        # print(f"{Xall['target_weight_s'][0].shape = }")
        # print(f"{Xall['target_weight_q'].shape = }")
        # print(f"{Xall['img_metas'] = }")

        # print(f"{self.db[query_id]['skeleton'] = }")

        # print(f"{sample_obj_list = }")
        # print(f"{query_obj = }")
